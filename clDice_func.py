...
@tf.function          # XLA → one fused GPU kernel
...
def soft_dilate(img):
    ...
    """
    if img.shape.rank == 4:  # N H W C
        return tf.nn.max_pool2d(img, ksize=3, strides=1, padding="SAME")
    else:  # N D H W C
        return tf.nn.max_pool3d(img, ksize=3, strides=1, padding="SAME")
...
@tf.function          # XLA ⇒ single fused kernel
...
def soft_clDice_loss(y_true, y_pred, iter_=None):
    ...
    """
    smooth = 1.0
    skel_pred = tf.stop_gradient(soft_skel(tf.stop_gradient(y_pred), iter_))
    skel_true = tf.stop_gradient(soft_skel(tf.stop_gradient(y_true), iter_))
