...
def soft_erode(img):
    ...
    """
    if len(img.shape) == 4:
        p2 = -KL.MaxPool2D(pool_size=(3, 1), strides=(1, 1), padding='same')(-img)
        p3 = -KL.MaxPool2D(pool_size=(1, 3), strides=(1, 1), padding='same')(-img)
        return tf.math.minimum(p2, p3)
    else:
        p1 = -KL.MaxPool3D(pool_size=(3, 3, 1), strides=(1, 1, 1), padding='same')(-img)
        p2 = -KL.MaxPool3D(pool_size=(3, 1, 3), strides=(1, 1, 1), padding='same')(-img)
        p3 = -KL.MaxPool3D(pool_size=(1, 3, 3), strides=(1, 1, 1), padding='same')(-img)
...
def soft_clDice_loss(y_true, y_pred, iter_=50):
    ...
    cl_dice = 1.0 - 2.0 * (pres * rec) / (pres + rec)
    return cl_dice
...
def soft_dice(y_true, y_pred):
    ...
    coeff = (2.0 * intersection + smooth) / (K.sum(y_true) + K.sum(y_pred) + smooth)
    return 1.0 - coeff
