...
class CycleGan():
    def __init__(
        ...
        self.subvol_patch_size = args.SUBVOL_PATCH_SIZE
        ...
        with self.strategy.scope():
            ...
            if self.wasserstein:
                ...
                # self.gen_A_optimizer = tf.keras.optimizers.Adadelta(clipnorm=10)
                # self.gen_B_optimizer = tf.keras.optimizers.Adadelta(clipnorm=10)
                # self.disc_A_optimizer = tf.keras.optimizers.Adadelta(clipnorm=10)
                # self.disc_B_optimizer = tf.keras.optimizers.Adadelta(clipnorm=10)
                
                # self.gen_A_optimizer = tf.keras.optimizers.RMSprop(learning_rate=2e-4, clipnorm=10)
                # self.gen_B_optimizer = tf.keras.optimizers.RMSprop(learning_rate=2e-4, clipnorm=10)
                # self.disc_A_optimizer = tf.keras.optimizers.RMSprop(learning_rate=2e-4, clipnorm=10)
                # self.disc_B_optimizer = tf.keras.optimizers.RMSprop(learning_rate=2e-4, clipnorm=10)
    ...
    def computeLosses(self, real_A, real_B, result, training=True):
        ...
        cycle_loss_B = self.cycle_loss_fn(self, real_A, cycled_A, typ='mse')
        ...
        # nreal_A = self.addNoise(real_A, 0.9 ** self.current_epoch)
        # nfake_A = self.addNoise(fake_A, 0.9 ** self.current_epoch)
        # disc_real_A = self.disc_A(nreal_A, training=training)
        # disc_fake_A = self.disc_A(nfake_A, training=training)
        
        # # nreal_B = self.addNoise(real_B, 0.9 ** self.current_epoch)
        # # nfake_B = self.addNoise(fake_B, 0.9 ** self.current_epoch)
        ...
        total_loss_A = gen_AB_loss + cycle_loss_A + seg_loss
        total_loss_B = gen_BA_loss + cycle_loss_B + perceptualA_loss
        ...
        result.update({
            ...
            # 'D_A_real_loss': reduce_mean(self, disc_real_A),
            # 'D_A_fake_loss': reduce_mean(self, disc_fake_A),
            # 'D_B_real_loss': reduce_mean(self, disc_real_B),
            # 'D_B_fake_loss': reduce_mean(self, disc_fake_B),
