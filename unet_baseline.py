...
separate folders for the imaging and segmentation domains.
...
python unet_baseline.py \
    ...
    --test-label-dir  /path/to/test/labels \
...
def prepare_datasets(
    ...
    train_images = _list_h5_files(train_image_dir)
    train_labels = _list_h5_files(train_label_dir)
    val_images = _list_h5_files(val_image_dir)
    val_labels = _list_h5_files(val_label_dir)

    if len(train_images) != len(train_labels):
        raise ValueError("Training image/label counts do not match")
    if len(val_images) != len(val_labels):
        raise ValueError("Validation image/label counts do not match")

    for img, lbl in zip(train_images, train_labels):
        if img.stem != lbl.stem:
            raise ValueError(f"Mismatched training pair: {img.name} vs {lbl.name}")
    for img, lbl in zip(val_images, val_labels):
        if img.stem != lbl.stem:
            raise ValueError(f"Mismatched validation pair: {img.name} vs {lbl.name}")

    train_ds = _build_tf_dataset(train_images, train_labels, patch_shape, batch_size)
    val_ds = _build_tf_dataset(val_images, val_labels, patch_shape, batch_size)
...
def train_unet(args: argparse.Namespace) -> tf.keras.Model:
    ...
    dataset = prepare_datasets(
        ...
        Path(args.val_image_dir),
        Path(args.val_label_dir),
        patch_shape,
        args.batch_size,
    )
    ...
    if args.output_dir:
        callbacks.append(tf.keras.callbacks.ModelCheckpoint(
            filepath=os.path.join(args.output_dir, "unet_weights.h5"),
            save_best_only=True,
...
def evaluate(model: tf.keras.Model, args: argparse.Namespace) -> List[dict]:
    patch_shape = tuple(args.patch_size)
    stride = tuple(args.inference_stride)

    def preprocess_fn(arr: np.ndarray) -> np.ndarray:
        tensor = tf.convert_to_tensor(arr, dtype=tf.float32)
        tensor = min_max_norm_tf(tensor)
        tensor = rescale_arr_tf(tensor, alpha=-0.5, beta=0.5)
        return tensor.numpy()

    image_paths = _list_h5_files(Path(args.test_image_dir))
    label_paths = _list_h5_files(Path(args.test_label_dir))
    if len(image_paths) != len(label_paths):
        raise ValueError("Test image/label counts do not match")
    for img, lbl in zip(image_paths, label_paths):
        if img.stem != lbl.stem:
            raise ValueError(f"Mismatched test pair: {img.name} vs {lbl.name}")
    ...
    for img_path, lbl_path in zip(image_paths, label_paths):
        ...
        labels = _load_full_volume(lbl_path, "label")
        if volume.ndim == 3:
        ...
        prediction = sliding_window_inference(
            ...
            preprocess=lambda arr: preprocess_fn(arr).astype(np.float32),
        )
        ...
        if args.save_predictions:
            pred_path = output_dir / f"{img_path.stem}_prediction.h5"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--train-image-dir", required=True, help="Directory of training image HDF5 volumes")
    parser.add_argument("--train-label-dir", required=True, help="Directory of training label HDF5 volumes")
    parser.add_argument("--val-image-dir", required=True, help="Directory of validation image HDF5 volumes")
    parser.add_argument("--val-label-dir", required=True, help="Directory of validation label HDF5 volumes")
    parser.add_argument("--test-image-dir", required=True, help="Directory of test image HDF5 volumes")
    parser.add_argument("--test-label-dir", required=True, help="Directory of test label HDF5 volumes")
